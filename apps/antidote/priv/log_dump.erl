...
usage() ->
    io:format("usage: log_dump LOG_FILE\n"),
    halt(1).

read_log(LogFile) ->
    case disk_log:open([{name, LogFile}]) of
        {ok, Log} ->
            io:format("FILE: ~p~nINFO: ~p~n",
                       [LogFile, disk_log:info(Log)]),
...
%    io:format("RRR ~p~p~n", [ record_info(fields, update_log_payload), R ]);
